# SRE Answer Assurance

A local, response-level evidence scorer. It reads frozen incident/log files, evaluates the initial todo plan, scores logical response-supporting steps, and writes JSON plus static HTML. It never executes the SRE agent's logged tools or writes to IcM.
...
**Start here:** [Local setup](#local-setup) | [Synthetic demo](#run-the-complete-synthetic-replay) | [Real pilot preparation](#prepare-the-real-pilot-roster) | [Remaining tasks](TASKS.md) | [Two-person workflow](CONTRIBUTING.md)
...
Local case manifest + frozen incident/log snapshots
  ...
  -> results.json + escaped static HTML
```
...
Requires Python 3.11+ and Git. Start in the standalone `sre-answer-assurance` repository, not the parent folder containing other repositories.
...
```powershell
git clone <YOUR_APPROVED_PRIVATE_REPOSITORY_URL>
cd sre-answer-assurance
python -m venv .venv
.\.venv\Scripts\python.exe -m pip install -c requirements-dev.lock -e ".[dev]"
```

On macOS/Linux, the virtual-environment interpreter is `.venv/bin/python`; use it in place of the Windows interpreter in the examples. Activation is optional. An activated environment also lets you use `python -m sre_assurance` directly.

No `.env`, Azure login, model key, or real incident export is needed to run unit tests and the synthetic demo.

On Windows, prefer a short checkout path such as `C:\repos\sre-answer-assurance`. Very deeply nested checkouts can exceed Windows path limits while installing Pyright's bundled type stubs. If installation reports a missing path deep inside `pyright`, retry from a shorter checkout path; no machine-wide setting change is required.
...
```powershell
.\.venv\Scripts\python.exe -m sre_assurance demo --directory data\demo
.\.venv\Scripts\python.exe -m sre_assurance validate --manifest data\demo\cases.json
.\.venv\Scripts\python.exe -m sre_assurance evaluate `
    --manifest data\demo\cases.json `
    --config data\demo\evaluation.json `
    --judge-mode replay --run-id demo
.\.venv\Scripts\python.exe -m sre_assurance serve --directory out\demo --port 8765
```
...
```powershell
.\.venv\Scripts\python.exe -m sre_assurance render --results out\demo\results.json
```
...
.\.venv\Scripts\python.exe -m sre_assurance prepare-corpus `
...
`tools\export-queries.kql` is a read-only preparation template. Run it manually in Application Insights and save **one table per file**; the evaluator itself has no live incident/Kusto client. Check the export's expected row counts so portal limits do not look like complete data.
...
Packaged prompts are in `src\sre_assurance\prompts`. Calls are cached by model/endpoint protocol, payload, prompt/schema, policy and inference settings. Successful cached responses are schema validated on reuse.
...
`out\<run-id>\results.json` preserves raw evidence references, gate votes, claims, step judgments and versions. `index.html` renders that result without live calls. Null scores remain null; a batch never hides unscorable/error cases or relabels them as score 0.
...
The localhost server exposes only report HTML/CSS; it does not serve incident JSON, corpus, caches, credentials, parent directories or arbitrary files. It refuses non-loopback hosts.
...
| Area | Purpose |
|---|---|
| `src\sre_assurance\imports.py`, `bundle.py`, `corpus.py` | Local collection preparation, raw imports, response selection and evidence normalization |
| `src\sre_assurance\documents.py` | Permitted document fetching, pinned snapshots and integrity-checked cache |
| `src\sre_assurance\judges.py`, `prompts` | Model protocols, strict judgment contracts and replay/cache |
| `src\sre_assurance\scoring.py`, `executor.py` | Gate and dimension aggregation, claim/step coordination |
| `src\sre_assurance\report.py`, `templates`, `assets` | Escaped HTML and loopback-only report serving |
...
GitHub Actions is configured to run publish-candidate checks, tests, type checking and package builds on Windows and Linux with Python 3.11/3.12 after the repository is pushed. It uses synthetic/mocked inputs only and does not require deployment secrets.

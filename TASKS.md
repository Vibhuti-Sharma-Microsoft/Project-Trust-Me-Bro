...
- JSON results, escaped HTML and loopback-only report serving.
...
**Not yet demonstrated:** scoring the ten real cases with approved live judges and reviewed production source policies. Incident IDs alone do not make a completed test corpus.
...
| EVAL-01 | Configure approved GPT, Claude and Gemini endpoints | B | Approved endpoints/model IDs/auth needed | All three role configurations work with a supported protocol; no public fallback or credential leakage |
...
| UI-01 | Refine real-data claim/evidence drill-down | B | EVAL-04 | A teammate can navigate response -> claim -> producing call/source -> judgment and understand every withheld field |
...
1. A starts DATA-01/02; B starts EVAL-01 and tests endpoint configuration against synthetic inputs.
